...
// --- LOOP ---
void loop() {
//niesprawdzone wygenerowane po zajeciach
  ...
  if (millis() - lastMeasure > 5000) {
    ...
    jsonDoc["ping"]       = pingOK ? "OK" : "FAIL";
    jsonDoc["temp_local"] = t_local;
    jsonDoc["pres_local"] = p_local;
    jsonDoc["fail_count"] = failCount;
    ...
    if (pingOK) {
      ...
      if (httpCode == HTTP_CODE_OK) {
        ...
        if (!error) {
          double t_api = apiDoc["current"]["temperature_2m"];
          jsonDoc["temp_api"] = t_api;
